...
func Parse(name, text, leftDelim, rightDelim string, symbols ...map[string]any) (map[string]*Tree, error) {
	treeSet := make(map[string]*Tree)
	t := New(name)
	t.text = text
	_, err := t.Parse(text, leftDelim, rightDelim, treeSet, symbols...)
	return treeSet, err
}
...
func New(name string, symbols ...map[string]any) *Tree {
	return &Tree{
		Name:    name,
		symbols: symbols,
	}
...
func (t *Tree) startParse(symbols []map[string]any, lex *lexer, treeSet map[string]*Tree) {
	t.Root = nil
	t.lex = lex
	t.vars = []string{"$"}
	t.symbols = symbols
...
// the treeSet map.
func (t *Tree) Parse(text, leftDelim, rightDelim string, treeSet map[string]*Tree, symbols ...map[string]any) (tree *Tree, err error) {
	defer t.recover(&err)
	t.ParseName = t.Name
	lexer := lex(t.Name, text, leftDelim, rightDelim)
	t.startParse(symbols, lexer, treeSet)
	t.text = text
	t.parse()
	t.add()
	t.stopParse()
	...
	for t.peek().typ != itemEOF {
		if t.peek().typ == itemLeftDelim {
			...
			if t.nextNonSpace().typ == itemDefine {
				...
				newT.startParse(t.symbols, t.lex, t.treeSet)
...
func (t *Tree) blockControl() Node {
	...
	block.startParse(t.symbols, t.lex, t.treeSet)
	var end Node
...
func (t *Tree) term() Node {
	...
	case itemIdentifier:
		...
		return t.newIdentifier(token.val, token.pos)
	case itemDot:
	...
	case itemPipeVariable:
		return t.newPipeVariable(token.pos, token.val)
	case itemField:
...
func (t *Tree) arguments() []Node {
	...
	for {
		...
		switch next := t.nextNonSpace(); next.typ {
		case itemRightParen:
			t.backup()
		case itemComma:
		default:
			t.unexpected(next, "arguments")
		}
