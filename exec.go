...
import (
	...
	"github.com/jinlongchen/teasy/fmtsort"
...
func (s *state) evalCommand(dot reflect.Value, cmd *parse.CommandNode, final reflect.Value) reflect.Value {
	...
	case *parse.IdentifierNode:
		// Must be a function.
		return s.evalIdentifierNode(dot, n, cmd, cmd.Args, final)
	case *parse.PipeNode:
...
func (s *state) evalChainNode(dot reflect.Value, chain *parse.ChainNode, args []parse.Node, final reflect.Value) reflect.Value {
	...
	case *parse.PipeVariableNode:
		return s.evalFieldChain(dot, final, chain, chain.Field, args, final)
	default:
		pipe := s.evalArg(dot, nil, chain.Node, final)
...
func (s *state) evalPipeVariableNode(dot reflect.Value, variable *parse.PipeVariableNode, args []parse.Node, final reflect.Value) reflect.Value {
	// #.Field has # as the first ident, Field as the second. Eval the var, then the fields.
	s.at(variable)
	value := final
	if len(variable.Ident) == 1 {
		return value
	}
	return s.evalFieldChain(dot, value, variable, variable.Ident[1:], args, missingVal)
}
...
func (s *state) evalIdentifierNode(dot reflect.Value, node *parse.IdentifierNode, cmd parse.Node, args []parse.Node, final reflect.Value) reflect.Value {
	s.at(node)
	name := node.Ident

	symbol, isBuiltin, ok := findSymbol(name, s.tmpl)
	if ok {
		if symbol.Kind() == reflect.Func {
			return s.evalCall(dot, symbol, isBuiltin, cmd, name, args, final)
		}
		return symbol
	}
...
func (s *state) evalCall(dot, fun reflect.Value, isBuiltin bool, node parse.Node, name string, args []parse.Node, final reflect.Value) reflect.Value {
	...
	if isBuiltin && (name == "and" || name == "or") {
		...
		for _, arg := range args {
			...
			v = s.evalArg(dot, argType, arg, final).Interface().(reflect.Value)
			if truth(v) == (name == "or") {
	...
	for ; i < numFixed && i < len(args); i++ {
		...
		case *parse.PipeVariableNode:
			argv = append(argv, s.evalArg(final, typ.In(i), arg, final))
		default:
			argv = append(argv, s.evalArg(dot, typ.In(i), arg, final))
		}
	}
	// Now the ... args.
	if typ.IsVariadic() {
		...
		for ; i < len(args); i++ {
			switch arg := args[i].(type) {
			case *parse.PipeVariableNode:
				argv = append(argv, s.evalArg(final, argType, arg, final))
			default:
				argv = append(argv, s.evalArg(dot, argType, arg, final))
			}
		}
	}
	// Add final value if necessary.
	if !isMissing(final) && i == 0 {
		t := typ.In(0)
		if typ.IsVariadic() {
			if len(argv) < numFixed {
		...
		argv = append(argv, s.validateType(final, t))
	}
...
func (s *state) evalArg(dot reflect.Value, typ reflect.Type, n parse.Node, final reflect.Value) reflect.Value {
	s.at(n)
	switch arg := n.(type) {
	case *parse.DotNode:
	...
	case *parse.FieldNode:
		return s.validateType(s.evalFieldNode(dot, arg, []parse.Node{n}, final), typ)
	case *parse.VariableNode:
		return s.validateType(s.evalVariableNode(dot, arg, nil, final), typ)
	case *parse.PipeVariableNode:
	...
	case *parse.ChainNode:
		return s.validateType(s.evalChainNode(dot, arg, nil, final), typ)
	}
	switch typ.Kind() {
	case reflect.Bool:
...
func (s *state) evalEmptyInterface(dot reflect.Value, n parse.Node) reflect.Value {
	...
	case *parse.IdentifierNode:
		return s.evalIdentifierNode(dot, n, n, nil, missingVal)
	case *parse.NilNode:
